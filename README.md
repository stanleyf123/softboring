...
- `/en/account` and `/zh-tw/account` (plan badge, review count, a vague signed-in desk count, upgrade, weekly reminder, timezone, soft leave)
...
- `/en/wall/saved` and `/zh-tw/wall/saved` (Soft+ private Soft Wall bookmarks, plus a JSON export of bookmark ids)
...
Login, register, forgot-password, and OAuth start/callback are **rate-limited** by IP (and email where it applies) in SQLite (`rate_limits`). Too many tries return HTTP 429 or send you back to login with a calm message. Soft Wall requests that return 429 (quote cards, echoes, and any other wall call) open a cream toast instead of a sharp error. No extra env is required.

The public app is installable as a **PWA** (`/manifest.webmanifest`, icons under `/icons/`, service worker `/sw.js`). On a phone, browsers that can install (or iOS Add to Home Screen) may see one calm tip; dismissing it keeps it dismissed. The worker does not cache `/api/*` or `/admin`, so sessions stay on the network. A local offline banner notices when the device drops offline and when it comes back — no request, no account.

SEO: unique titles/descriptions, canonicals, and `hreflang` (`en` / `zh-TW` / `ja` / `x-default`) on public pages; `/sitemap.xml` and `/robots.txt` (allow public, disallow `/admin`, `/api/`, `/account`). JSON-LD is Organization / WebSite / SoftwareApplication with no invented ratings. Optional `GOOGLE_SITE_VERIFICATION` / `BING_SITE_VERIFICATION`. Operator checklist: [docs/seo.md](./docs/seo.md). Japanese and invites: [docs/ja-and-invites.md](./docs/ja-and-invites.md). Soft Wall filters, monthly digest, seasonal packs: [docs/engagement-trio.md](./docs/engagement-trio.md). Soft year, wall compliments, mid-week notes: [docs/soft-year-activity-notes.md](./docs/soft-year-activity-notes.md). Soft postcard, wall spotlight, week compare: [docs/soft-postcard-spotlight-compare.md](./docs/soft-postcard-spotlight-compare.md). Soft intention, sticker pocket UX, Soft Wall a11y: [docs/soft-intention-wall-a11y.md](./docs/soft-intention-wall-a11y.md). Soft bookmarks, empty-state illustrations, Soft tips inbox: [docs/soft-bookmarks-empty-tips.md](./docs/soft-bookmarks-empty-tips.md). Soft print week, wall color preference, onboarding checklist: [docs/soft-print-wall-color-checklist.md](./docs/soft-print-wall-color-checklist.md). Soft memory resurfacing, quiet writing, Soft Wall flags: [docs/soft-memory-quiet-wall-report.md](./docs/soft-memory-quiet-wall-report.md). Soft streak polish, Soft Wall hide-demo, soft shortcuts: [docs/streak-demo-shortcuts.md](./docs/streak-demo-shortcuts.md). Soft Wall guidelines, Soft+ gift codes, homepage soft-stats: [docs/guidelines-gifts-soft-stats.md](./docs/guidelines-gifts-soft-stats.md). Soft+ gift expiry UX, Soft Wall sort, gift thank-you: [docs/soft-expiry-wall-sort-thanks.md](./docs/soft-expiry-wall-sort-thanks.md). Soft Wall mood palette, free JSON download, footer cluster: [docs/soft-palette-export-footer.md](./docs/soft-palette-export-footer.md). Soft leave, timezone, PWA install tip: [docs/soft-leave-tz-pwa.md](./docs/soft-leave-tz-pwa.md). Soft offline banner, free review templates, cream week compare: [docs/soft-offline-templates-compare.md](./docs/soft-offline-templates-compare.md). Soft Wall guest search, account size note, pin-limit copy: [docs/soft-wall-search-size-pins.md](./docs/soft-wall-search-size-pins.md). Soft FAQ, wall-note hover, thank-you history: [docs/soft-faq-hover-thanks.md](./docs/soft-faq-hover-thanks.md). Sitemap polish, guest wall spotlight, Soft+ activity JSON: [docs/soft-sitemap-spotlight-activity.md](./docs/soft-sitemap-spotlight-activity.md). Wall-note share images, quiet review dots, Soft+ gratitude cards: [docs/soft-og-progress-gratitude.md](./docs/soft-og-progress-gratitude.md). Footer analytics note, Soft Wall batches, past digest months: [docs/soft-privacy-wall-digest.md](./docs/soft-privacy-wall-digest.md). Cream not-found page, signed-in desk count, Soft+ bookmark ids: [docs/soft-404-sessions-bookmarks.md](./docs/soft-404-sessions-bookmarks.md). Cream wall rate toasts, public nicknames, letters to a past week: [docs/soft-rate-nicknames-letters.md](./docs/soft-rate-nicknames-letters.md). Gift-code empty guidance, review draft restore, Soft+ letter inbox: [docs/soft-gifts-drafts-inbox.md](./docs/soft-gifts-drafts-inbox.md). Quiet security headers, Escape on Soft Wall, Soft+ checklist JSON: [docs/soft-security-escape-reflection.md](./docs/soft-security-escape-reflection.md). Demo neighbor badges, a gentle review-length hint, earlier soft intentions: [docs/soft-demo-hint-intention.md](./docs/soft-demo-hint-intention.md). Cream Soft Wall skeletons, a timezone-change confirm, Soft+ intention JSON: [docs/soft-skeleton-timezone-intention.md](./docs/soft-skeleton-timezone-intention.md). Open-week navigation, free seasonal question phrasing, Soft+ wall week chips: [docs/soft-week-nav-season-chips.md](./docs/soft-week-nav-season-chips.md). Nickname chips that open wall search, quiet week-chip empties, Soft+ intention distance and copy: [docs/soft-nick-week-intention.md](./docs/soft-nick-week-intention.md). Soft+ note-word search, a cream trends empty, and a mobile wall filter bar: [docs/soft-search-trends-sticky.md](./docs/soft-search-trends-sticky.md). Guest sky and paper color key, nickname confirm, and Soft Wall filter keys: [docs/soft-sky-nickname-keys.md](./docs/soft-sky-nickname-keys.md). History grouped by month, Soft+ report bars, and quieter links to pause or leave: [docs/soft-months-report-edges.md](./docs/soft-months-report-edges.md).

Weekly reminders: on the account page, toggle a weekday and an IANA timezone (default `Asia/Taipei`). Changing the timezone asks for a soft confirm first, because reminders and the digest follow that clock. Changing the public nickname also waits for a soft confirm, because neighbors see that name on the wall. Cron later with `npm run reminders:dispatch` still wakes on the VPS clock, then checks that weekday in the member’s timezone (**no-op success** if email env is missing). The same timezone bounds the digest’s “this month.” Signed-in members can delete the account from that page (type `Soft Boring` and their email); reviews and Soft Wall notes leave with them. See [DEPLOY-LINODE.md](./DEPLOY-LINODE.md).

## Plans

| Plan | Write reviews | History | Trends | Soft Wall |
| --- | --- | --- | --- | --- |
| **Free** (and guests) | Yes (+ private mid-week soft note and soft weekly intention when signed in). A long field gets a gentle hint and still saves. The same six questions can be heard in this season’s phrasing for the visit; answers still save in the usual fields. Earlier soft intentions, and a JSON of those sentences, are a Soft+ tease | Latest **4** reviews stay open; older ones show a Soft+ prompt. An open week can step to the earlier or later open week. Tucked weeks are not linked. Signed-in Free can download those four as JSON. The soft checklist stays out of that file. Soft activity stays readable; its JSON download waits on Soft+. A letter back to an earlier week, and the inbox of those letters, are Soft+ teases | Locked (year / digest teasers; both pages are public in the sitemap) | Locked teaser plus one rotating public note, and a free nickname search on notes already visible. Homepage nickname chips open that same search. Demo neighbor notes wear a cream badge, even while the words stay blurred. The first paint is a cream skeleton, so the corkboard does not open blank. Sharing those open weeks is allowed; raising a note above the others stays on Soft+. A crowded wall shows a cream pause instead of a sharp error. Week chips stay with Soft+. Guests get a color key for the still sky and the six shared paper colors; personal washes stay off that key |
| **Soft+** | Yes | Unlimited, step between open weeks, search, export CSV/PDF, weekly JSON that includes the optional soft checklist, soft postcard PNG + print-this-week PDF, compare weeks, custom questions, monthly digest with earlier months, soft year, soft memory resurfacing, a short thank-you history, soft activity JSON, a short in-app letter to a past week, plus a private inbox of those letters on the account page, and a private list of earlier soft intentions, with a weeks-ago label and a copy-on-this-device button, plus a JSON download of those earlier sentences | Feeling 1–5 over time, plus a gentle weekly streak. An empty chart is a cream card: one saved feeling is enough for the first dot | Full access: read, drag, comment, one-level replies, stickers, badge, pin (one raised note), preferred note color, mood palette legend, private bookmarks (`/wall/saved`) with a JSON export of bookmark ids and collection membership, compliments feed, weekly soft picks, quiet “this feels off” flags. Week chips can rest the board on this ISO week or on earlier weeks, using the account timezone. On a phone, those chips wrap, and a small cream bar stays with the corkboard while it scrolls. Search reads the nickname and the words already on a readable note. When that view is empty, the board says so and can return to all weeks. Older weeks can be pinned to the wall. Readable notes open a little more on hover or focus. Escape folds that open note, and still closes the note dialog and sticker shop. Demo neighbor notes wear the same cream badge. Focused week chips take arrow keys. `[` `]` step the lowest feeling, and `{` `}` step the highest |
...
- **Free / logged-out** visitors see a locked teaser: positions, colors, and nicknames. Review text and comments are omitted by the API, not just blurred in CSS. A free search box filters those visible notes by nickname (and by words only when a note already carries them). It does not unlock the Soft+ feeling, sort, or demo filters.
- Signed-in desks see a soft size note on Account: how many reviews and Soft Wall notes (including notes taken down) this visit is keeping. Nothing is emailed.
- Sharing a week explains the pin window: Free can pin the latest four open weeks; Soft+ can pin older weeks too. Raising a note above the others stays at one, and only on Soft+.
