  ...
  return (
    <header className={`${SITE_SHELL_CLASS} py-4 md:py-5 print:hidden`}>
      <div className="flex items-center justify-between gap-3 rounded-[1.75rem] bg-paper/90 px-3 py-2 shadow-card backdrop-blur-sm md:gap-4 md:rounded-full md:px-5">
        <Link
          href="/"
          className={`${SOFT_CHROME_FOCUS} flex shrink-0 items-center gap-2.5 rounded-full font-display text-lg tracking-tight text-foreground`}
        >
          ...
          >
            {links.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={
                    active
                      ? `${SOFT_CHROME_FOCUS} rounded-full bg-peach px-2.5 py-1 text-foreground`
                      : `${SOFT_CHROME_FOCUS} rounded-full px-2.5 py-1 hover:text-foreground`
                  }
                >
                  {t(link.key)}
                </Link>
            ...
            {softPlus ? (
              <Link
                ...
                className={
                  trendsActive
                    ? `${SOFT_CHROME_FOCUS} rounded-full bg-mint px-2.5 py-1 text-foreground`
                    : `${SOFT_CHROME_FOCUS} rounded-full px-2.5 py-1 hover:text-foreground`
                }
              >
                {t("trends")}
            ...
            {softPlus ? (
              <Link
                ...
                className={
                  yearActive
                    ? `${SOFT_CHROME_FOCUS} rounded-full bg-blush px-2.5 py-1 text-foreground`
                    : `${SOFT_CHROME_FOCUS} rounded-full px-2.5 py-1 hover:text-foreground`
                }
              >
                {t("year")}
            ...
            {softPlus ? (
              <Link
                ...
                className={
                  digestActive
                    ? `${SOFT_CHROME_FOCUS} rounded-full bg-lemon px-2.5 py-1 text-foreground`
                    : `${SOFT_CHROME_FOCUS} rounded-full px-2.5 py-1 hover:text-foreground`
                }
              >
                {t("digest")}
