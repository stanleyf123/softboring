...
type SoftIntentionPayload = {
  weekKey: string;
  maxLength: number;
  intention: { id: string; body: string; weekKey: string } | null;
};

export function SoftIntentionCard({
  signedIn,
  variant = "review",
}: {
  signedIn: boolean;
  ...
  const [maxLength, setMaxLength] = useState(SOFT_INTENTION_MAX);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = (await response.json()) as SoftIntentionPayload;
        if (cancelled) return;
        setWeekKey(data.weekKey);
        setMaxLength(data.maxLength || SOFT_INTENTION_MAX);
        setBody(data.intention?.body ?? "");
        setLoaded(true);
      } catch {
        if (!cancelled) setLoadError(true);
      }
  ...
  const shell =
    variant === "account"
      ? "mt-8 rounded-[1.5rem] bg-lemon/40 px-5 py-5"
      : "rounded-[1.75rem] bg-lemon/35 px-6 py-6 shadow-card sm:px-8";
  ...
  async function save() {
    ...
    setSaveError(false);
    try {
      const response = await fetch("/api/soft-intentions", {
        method: "PUT",
  ...
  return (
    <section id="soft-intention" className={`${shell} scroll-mt-24`}>
      ...
      <p className="mt-4 text-xs leading-relaxed text-muted">{t("hint")}</p>
    </section>
  );
}

export function LastIntentionNudge({ signedIn }: { signedIn: boolean }) {
  const t = useTranslations("SoftIntention");
  const hydrated = useHydrated();
  const [last, setLast] = useState<{ body: string; weekKey: string } | null>(null);
    ...
    (async () => {
      try {
        ...
        const data = (await response.json()) as {
          last?: { body: string; weekKey: string } | null;
        };
        if (!cancelled && data.last?.body) {
          setLast({ body: data.last.body, weekKey: data.last.weekKey });
        }
      } catch {
        // Soft nudge — ignore load failures.
      }
    })();
    return () => {
      cancelled = true;
  ...
  if (!signedIn || !hydrated || !last) return null;

  return (
    <aside
      className="rounded-[1.5rem] border border-line/70 bg-paper/70 px-5 py-4"
      aria-label={t("lastAria")}
    >
      <p className="text-xs uppercase tracking-wide text-muted">{t("lastEyebrow")}</p>
      <p className="mt-2 text-sm leading-relaxed text-muted">{t("lastLead")}</p>
      <p className="mt-2 font-display text-base tracking-tight">“{last.body}”</p>
      <p className="mt-2 text-xs text-muted">{t("weekLabel", { week: last.weekKey })}</p>
    </aside>
  );
}
